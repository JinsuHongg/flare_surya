...
class GlobalR2Score(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        # We need a two-pass or delayed approach for SS_tot if we want the true mean.
        # Alternatively, use: SS_tot = sum(y^2) - (sum y)^2 / n
        # Let's track sum_y_sq too.
        if not hasattr(self, "sum_y_sq"):
