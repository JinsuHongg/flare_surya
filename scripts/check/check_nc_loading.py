...
def run(filepaths: list[str], channels: list[str]):
    ...
    for i, fp in enumerate(filepaths):
        ...
        print(
            f"[{i+1}/{len(filepaths)}] {' | '.join(parts)}  sum={ref:.4e} {'✓' if ok else '✗'}"
        )
    ...
    for name in LOADERS:
        ...
        print(
            f"{name:15s} {t.mean():7.3f}s {t.min():7.3f}s {t.max():7.3f}s {t.sum():8.3f}s {sp:7.1f}x"
        )
...
def main():
    ...
    p.add_argument(
        "--index_csv",
        default="~/project/flare_surya/flare_surya/data/surya_input_data.csv",
    )
