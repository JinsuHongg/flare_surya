...
def main(new_data_path, tag):
    ...
    df_leaky_val = pd.read_csv(
        (new_data_path.parent / "leaky_validation.csv").resolve()
    )
    ...
    df_new_train = pd.merge(
        df_new, df_train[["timestamp"]], on="timestamp", how="inner"
    )
    df_new_val = pd.merge(
        df_new, df_validation[["timestamp"]], on="timestamp", how="inner"
    )
    df_new_leaky_val = pd.merge(
        df_new, df_leaky_val[["timestamp"]], on="timestamp", how="inner"
    )
