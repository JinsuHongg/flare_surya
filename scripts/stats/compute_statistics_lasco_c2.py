...
def compute_statistics(
    ...
    if index_path is not None:
        ...
        try:
            ...
            # Sample matched_indices to keep computation within reasonable limits
            if len(matched_indices) > 10000:
                logger.info(f"Sampling {10000} indices out of {len(matched_indices)} for statistics")
                matched_indices = np.random.choice(matched_indices, 10000, replace=False)
            
            logger.info(
                f"Found {len(matched_indices)} common timestamps after matching/sampling."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--zarr_path",
        type=str,
        default="/media/jhong90/storage/surya/soho_lasco_c2_2010_2025_v1.zarr",
